...
bool input_coefficients_or_exit(coefs_roots *data)
{
        ...
        printf(ANSI_YELLOW "Enter the coefficients for the quadratic equation a*x^2 + b*x + c = 0 separated by a space\n" ANSI_DEFAULT_COLOUR);
...
user_choice check_input(int count_input)
{
        if (!is_input_empty() or count_input != 3 or count_input == EOF) {
                int x = -1;
                while (true) {
                        printf (ANSI_LIGHT_GREEN "Incorrect input, input: 0 - to exit, 1 - to continue\n" ANSI_DEFAULT_COLOUR);
                        int count_x = scanf("%d", &x);

                        if (!(!is_input_empty() or count_x != 1 or count_x == EOF or (x != 1 and x != 0))) {
                            break;
                        }
                }
                if (x == 0)
